...
React がコンポーネントツリーの差分計算（リコンシリエーション）を行う内部アーキテクチャ。

**Stack Reconciler（React 15 以前）の問題**

再帰呼び出しで同期的に差分計算を行っていたため、コールスタックに積まれた処理を途中で止められなかった。大きなツリーでは JS スレッドを長時間占有し、ユーザー入力やアニメーションが待ち状態になるパフォーマンス問題が起きた。

**Fiber Reconciler が解決したこと**

各コンポーネントを「Fiber ノード」という単位で管理し、リンクリスト構造（`child` / `sibling` / `return` ポインタ）でツリーを表現した。ループで処理するため「どこまで処理したか」を保持でき、任意の場所で中断・再開が可能になった。

```
Stack Reconciler          Fiber Reconciler
─────────────────         ─────────────────
再帰 → 途中停止不可        ループ → 任意の場所で中断可

コールスタック              Fiber ノード（リンクリスト）
┌─────────┐               [App] → [Header] → [Nav] → ...
│  Item   │                 ↑
│  List   │               中断した場所から再開できる
│   Nav   │
...
Scheduler が更新の優先度を決め、Fiber Reconciler がノード単位で差分計算を中断・再開しながら処理する。`useTransition` で「緊急度低」とマークした更新を後回しにできるのはこの仕組みのため。

### 自動バッチング

複数の `setState` をまとめて1回のレンダーにする最適化。

**React 17 以前の仕組み**

React は独自のイベントシステム（合成イベント）を持ち、イベントハンドラ実行時に内部で「バッチングフラグ」を ON にしていた。`setState` はフラグが ON の間だけキューに積まれ、フラグが OFF になったタイミングでまとめてレンダーされる。`setTimeout` / `Promise.then` のコールバックはイベントハンドラのスコープ外で実行されるためフラグが OFF のまま → `setState` が呼ばれるたびに即レンダーが走る。

**React 18 からの変更**

Scheduler ベースの仕組みに変更し、「React のフラグの中かどうか」ではなく「同一タスク内の更新はすべてまとめる」方針になった。`setTimeout` / `Promise.then` / ネイティブイベント内でも自動バッチが効く。

意図的に個別レンダーが必要な場合は `flushSync` を使う。

```ts
import { flushSync } from 'react-dom'
flushSync(() => setCount(c => c + 1)) // ここでレンダー
flushSync(() => setFlag(f => !f))     // ここでレンダー
```
...
開発環境で `useEffect` が「マウント → アンマウント → 再マウント」と2回実行されるようになった。

**なぜ2回実行するのか**

冪等性の観点から、何度マウント・アンマウントを繰り返しても副作用が壊れないように書くことを強制するため。将来の React がオフスクリーンレンダリング（タブ切り替え時にコンポーネントを破棄せず再利用）を実装する際に複数回マウントが起きることを想定している。本番環境では発生しない（`next dev` のみ）。
